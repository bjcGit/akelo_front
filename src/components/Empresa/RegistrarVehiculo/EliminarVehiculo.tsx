...
interface EliminarVactorProps {
  id: string;
  refreshTable: () => void;
}

export const EliminarVehiculo: React.FC<EliminarVactorProps> = ({
  id,
  refreshTable,
}) => {
  ...
  const handleDelete = async () => {
    try {
      ...
      const response = await axios.delete(`${baseUrl}/vehiculo/${id}`);
      ...
      Notify.failure("Error al eliminar el vehículo");
    } finally {
  ...
  return (
    <React.Fragment>
      ...
      >
        <DialogTitle>{"Desea eliminar este vehículo?"}</DialogTitle>
        <DialogContent>
          <DialogContentText id="alert-dialog-slide-description">
            Al darle click al boton "Eliminar" está borrando por completo este
            vehiculo y su informacion.
          </DialogContentText>
        </DialogContent>
        <DialogActions>
          <Button
